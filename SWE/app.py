...
def getPrediction():
    ...
    if regression == "Linear":
        ...
        return render_template("results.html", msg = userMsg, prediction = predictions[0], r2 = results['R2'], sse = results['SSE'])
    else:
        if sex == 'Male': #selecting correct model and data based on sex
            vert_data = vert_data_M 
            gender = 'M'
        ...
        return render_template("results.html", msg = userMsg, prediction = predicted_sum_verts, r2 = results_rf[gender]["R-squared (Cross-Validation)"], sse = results_rf[gender]["Mean Squared Error (Cross-Validation)"])
